...
public class FriendshipManagement {
    ...
    public boolean sendFriendRequest(User requestSender,User requestReceiver){
        if(!requestSender.isUserBlocked(requestReceiver)){
           ...
           requestReceiver.getNotificationManager().addFriendRequestNotification(requestSender.getUserId()+" "+ requestReceiver.getUserId()+" | You have Recieved a Friend request From: "+requestSender.getUsername());
